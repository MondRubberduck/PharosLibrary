...
# --------------------------------------------------------------------------- #
# role vocabulary + packed channels
# --------------------------------------------------------------------------- #
#
# The rules live in service/asset_service/material_roles.py (stdlib only, no
# `unreal`), shared with the registry importer so both classify a parameter
# name identically.  Path bootstrap as in verify_pack_export.py.
_SVC = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__)))), "service")
if _SVC not in sys.path:
    sys.path.insert(0, _SVC)
from asset_service.material_roles import (  # noqa: E402
    PACKED_HINT_TOKENS, PACKED_ORDER_NOTE, PACKED_ORDER_TOKENS, ROLE_ORDER,
    ROLE_RULES, classify_param, packed_for)
