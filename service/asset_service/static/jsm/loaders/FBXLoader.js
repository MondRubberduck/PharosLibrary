...
class BinaryParser {
	...
	parse( buffer ) {
		...
		while ( ! this.endOfContent( reader ) ) {
			...
			// tolerate files with data appended after the footer (truncated or
			// concatenated downloads): a top-level node header whose endOffset
			// exceeds the buffer cannot be real -- stop instead of drifting
			if ( reader.offset + ( version >= 7500 ? 8 : 4 ) <= reader.size() ) {

				const eo = ( version >= 7500 )
					? Number( reader.dv.getBigUint64( reader.offset, true ) )
					: reader.dv.getUint32( reader.offset, true );
				if ( eo > reader.size() ) break;
	...
	parseProperty( reader ) {
		...
		switch ( type ) {
			...
			default:
				throw new Error( 'THREE.FBXLoader: Unknown property type ' + type + ' (charCode ' + type.charCodeAt( 0 ) + ') at file offset ' + reader.getOffset() );
